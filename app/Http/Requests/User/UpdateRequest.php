...
class UpdateRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'number_document' => ($this->type_document == 'DNI' ? 'required|min:9|unique:people,number_document,'.$this->people_id.'|ends_with:A,B,C,D,E,F,G,H,I,J,Q,L,M,N,O,P,Q,R,S,T,U,V,W,X,Y,Z' : ($this->type_document == 'RUC' ? 'required|min:13|unique:people,number_document,'.$this->people_id.'|ends_with:001' : 'required|min:10|unique:people,number_document,'.$this->people_id)),
            'direction' => 'required',
            'phone' => 'required|min:12',
            'email' => 'required|unique:people,email,'.$this->people_id.'|email',
